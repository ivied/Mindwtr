...
export class CardStore {
  ...
  /** Task revision result: replace open list, append moved-out items to closed_tasks. */
  setTasks(slug: string, openTasks: string, closedAdditions: string, seenAt: string): void {
    const c = this.get(slug)
    if (!c) return
    const closed = closedAdditions
      ? (c.closed_tasks ? c.closed_tasks + '\n' : '') + closedAdditions
      : c.closed_tasks
    this.db
      .query(`UPDATE entity_cards SET open_tasks=?, closed_tasks=?, updated_at=? WHERE slug=?`)
...
// ---------------- TaskRevisor (periodic open_tasks compaction) ----------------

const REVISE_TOOL = {
  type: 'function' as const,
  function: {
    name: 'revise_tasks',
    description: 'Split an accumulated open-tasks list into still-open items and done/stale items.',
    parameters: {
      type: 'object',
      properties: {
        open_tasks: {
          type: 'string',
          description:
            'Markdown bullets: ONLY items that are clearly still open actionable work. Merge duplicates/rewordings of the same task into one bullet. Keep original wording where possible.',
        },
        closed_tasks: {
          type: 'string',
          description:
            'Markdown bullets: items moved out — completed (per the timeline), superseded by a newer item, or stale one-off mentions never seen again. Prefix each with "done:" or "stale:". Empty if none.',
        },
      },
      required: ['open_tasks', 'closed_tasks'],
    },
  },
}

const REVISE_SYSTEM = `You revise the accumulated open-tasks list of ONE entity in a developer's knowledge base. The list grew over weeks and contains duplicates, completed items, and stale one-off mentions.

Rules:
- NEVER invent new tasks. Every output bullet must come from the input list (merged wording is fine).
- A task is done if the recent timeline shows it happened. A task is stale if it was a one-off mention with no follow-up. When unsure, keep it open — do not silently drop items: everything removed from open_tasks must appear in closed_tasks.
- Merge bullets that describe the same work.
Call revise_tasks exactly once.`

export class TaskRevisor {
  constructor(
    private readonly llm: LLMClient,
    private readonly model?: string
  ) {}
  ...
  ): Promise<{ open_tasks: string; closed_tasks: string } | null> {
    const user = `ENTITY: ${slug} [${type}]

RECENT TIMELINE (for judging what got done):
${recentTimeline || '(none)'}

ACCUMULATED OPEN_TASKS:
${openTasks}
...
export function taskLines(s: string): number {
  return s.split('\n').filter((l) => l.trim()).length
}

export function topTasks(s: string, n = 10): string {
  const lines = s.split('\n').filter((l) => l.trim())
